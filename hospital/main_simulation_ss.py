...
def plot_primary_results(history, forecasts):
    ...
    ax1.set(xlabel='Hour of Day', ylabel='Temperature (°C)', title='Thermal Performance (1-Minute Resolution)')
    ax1.grid(True)
    ax2 = ax1.twinx()
    ...
    ax3.set(ylabel='Power (kW)', title='Optimal Power Dispatch (1-Minute Resolution)')
    ...
    ax4.set(xlabel='Hour of Day', ylabel='State of Charge (%)', title='HESS Operation: BESS for Arbitrage, SC for Fluctuations', ylim=(0, 105))
    ax4.grid(True)
    ax4.legend()
    plt.tight_layout()
...
def plot_error_correction(history):
    ...
    ax.set(xlabel='Hour of Day', ylabel='Power (kW)', title='Level 2 MPC: Real-Time Error Correction by HESS')
    ax.grid(True)
    ax.legend()
...
def plot_additional_visualizations(history, forecasts, p_bess_plan, p_hvac_plan, baseline_cost):
    ...
    ax1.plot(hours, history['cost'], 'r-', lw=2, label='With MPC Control')
    ax1.plot(hours, baseline_cost, 'b--', lw=2, label='Without MPC (Grid + PV Only)')
    ax1.set(xlabel='Hour of Day', ylabel='Cumulative Cost ($)', title='Cumulative Operational Cost: With vs. Without MPC')
    ax1.legend()
    ax1.grid(True)
    ...
    ax2.set(ylabel='PV Power (kW)', title='Forecast vs. Actual Power')
    ax2.legend()
    ax2.grid(True)
    ...
    ax4.set(xlabel='Hour of Day', ylabel='BESS Power (kW)', title='Planned vs. Actual BESS Dispatch')
    ax4.legend()
    ax4.grid(True)
    ...
    ax5.set(xlabel='Hour of Day', ylabel='Power (kW)', title='Individual HESS Power Profiles (Charge/Discharge)')
    ax5.legend()
    ax5.grid(True)
